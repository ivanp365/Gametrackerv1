import { useState, useEffect } from 'react' // Se agregó el useEffect faltante
...
export default function Favoritos() {
  ...
  const eliminarFavorito = (id) => {
    const nuevos = favorites.filter(f => f.id !== id)
    setFavoritos(nuevos)
    if (user?.username) {
  ...
  >
    💝 Lista de Deseos
          ...
          <a href="/favoritos" className="flex items-center gap-3 text-white bg-purple-600/20 border border-purple-500/30 rounded-lg px-4 py-3 text-sm">⭐ Favoritos</a>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        <div className="mb-6">
          ...
          <p className="text-gray-400 mt-1">{favorites.length} juegos en tu biblioteca</p>
        </div>
